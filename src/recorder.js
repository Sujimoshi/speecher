...
class AudioRecorder extends require('events').EventEmitter {
  ...
  start () {
    ...
    let path
    if (this.options.paths && this.options.paths[this.options.program]) {
      path = this.options.paths[this.options.program]
      // Mac uses:
      // rec: '/usr/local/bin/rec'
      // console.log('processSpawn', path, this.command.arguments, this.command.options);
    } else {
      // assume global
      path = this.options.program
    }

    // this.childProcess = processSpawn(this.options.program, this.command.arguments, this.command.options);
    this.childProcess = processSpawn(path, this.command.arguments, this.command.options)
